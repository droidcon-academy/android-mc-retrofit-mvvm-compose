...
class MovieDetailsViewModel @Inject constructor(
    ...
    private fun getSimilarMovies(movieId: Int) {
        viewModelScope.launch {
            ...
            moviesRepository.getSimilarMovies(movieId = movieId).collect { result ->
                when (result) {
                    is Resource.Success -> {
                        val similarMovies = result.data?.results?.map {
                            it.copy(
                        ...
                        _eventFlow.emit(
                            UiEvents.SnackBarEvent(
                                message = result.message
                                    ?: SOMETHING_WENT_WRONG_WHEN_FETCHING_SIMILAR_MOVIES,
                            ),
                        )
                    }
